...
@"<packages>
  ...
  <package id=""packageB"" version=""2.2.0"" />
    ...
    <ItemGroup>
        ...
        public async Task MsbuildRestore_WithPackagesToPrune_PrunesCorrectly(bool isStaticGraphRestore, bool useLegacyDependencyResolver)
        {
            // Arrange
            using var pathContext = new SimpleTestPathContext();
            // Set up solution, project, and packages
            ...
            Path.Combine(pathContext.SolutionRoot, "Directory.Build.props"),
              ...
              <ItemGroup>
                <PrunedPackageReference Include=""y"" Version=""2.0.0"" />
              </ItemGroup>
            </Project>");
            ...
            // Act
            CommandRunnerResult result = _msbuildFixture.RunMsBuild(pathContext.WorkingDirectory, $"/t:restore {project.ProjectPath} " +
                (useLegacyDependencyResolver ? " /p:RestoreUseLegacyDependencyResolver=true" : string.Empty) +
                (isStaticGraphRestore ? " /p:RestoreUseStaticGraphEvaluation=true" : string.Empty),
                ignoreExitCode: true,
                testOutputHelper: _testOutputHelper);
