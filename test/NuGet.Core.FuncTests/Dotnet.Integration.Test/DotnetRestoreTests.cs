...
@$"<Project Sdk=""Microsoft.NET.Sdk"">
    ...
    <Target Name=""AssertRestoreTaskOutputProperties""
        ...
        public async Task DotnetRestore_WithConditionalPrunedPackageReference_Succeeds(bool isStaticGraphRestore, bool useLegacyDependencyResolver)
        {
            using SimpleTestPathContext pathContext = _dotnetFixture.CreateSimpleTestPathContext();
            var projectName = "ClassLibrary1";
            var workingDirectory = Path.Combine(pathContext.SolutionRoot, projectName);
            ...
            var result = _dotnetFixture.RunDotnetExpectSuccess(workingDirectory, $"restore {projectFile}" + (useLegacyDependencyResolver ? " /p:RestoreUseLegacyDependencyResolver=true" : string.Empty) +
                (isStaticGraphRestore ? " /p:RestoreUseStaticGraphEvaluation=true" : string.Empty), testOutputHelper: _testOutputHelper);
